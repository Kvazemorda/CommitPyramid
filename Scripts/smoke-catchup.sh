...
# Requires app to be runnable: swift run CommitPyramid.
#
# Setup:
#   1. Clean state:
#      rm -f ~/Library/Application\ Support/CommitPyramid/{events.jsonl,state.json,catchup-state.json}
#   2. Run with mock source:
#      CITY_SMOKE_CATCHUP=1 swift run CommitPyramid
#
# Expected:
...
#   cat ~/Library/Application\ Support/CommitPyramid/catchup-state.json | jq .
#   → should show:
#     {
#       "sources": {
...
#   1. Launch: CITY_SMOKE_CATCHUP=1 swift run CommitPyramid
#   2. Open Settings → set Catch-up interval to 3 min.
#   3. Observe errors.log if needed; after 3 min a new mock event fires.
#      tail -f ~/Library/Application\ Support/CommitPyramid/errors.log
#
# Normal mode (no mock):
