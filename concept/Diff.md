...
| D-16  | F-16 Расширенный каталог (50 юнитов)     | UnitKind 51 case + Legacy migration + Placeholder sprites + Terrain affinity + Evolution chains + UnitPlanner biome-aware + Stage-tier visuals + Документация ✅ через TASK-031/032/033/034/035/036/037/039. **Остаток:** тесты UnitPlanner (TASK-038, разблокирована); каталог промптов для AI-генерации спрайтов вместо самих PNG (TASK-040 переписан 2026-05-23 — продакшн-арта НЕ делаем сами, оставляем placeholder + PNG-first fallback, контрибьюторы добавят через PR). | TASK-038 переразобрать + TASK-040 в составе F-21 (docs/asset-prompts.md) | S |
...
| ID    | Фича (F-NN)                              | Что не так | Done-критерий                                                                | Объём |
|-------|------------------------------------------|------------|------------------------------------------------------------------------------|-------|
| D-23  | F-23 Cross-unit эволюция                 | Не реализовано — эволюция работает по простому kind-counter'у без учёта окружения. См. TASK-046, BUG-016 | EvolutionGraph с ≥10 правилами, requirements по соседним юнитам, deterministic replay | M |
| D-24  | F-24 Множитель веса коммита/задачи       | Не реализовано — нет настройки веса, по умолчанию слишком много юнитов. См. TASK-043, BUG-013 | Слайдер в Settings для git (default 0.1) и notes/tasks (default 1.0), применяется при reset | M |
