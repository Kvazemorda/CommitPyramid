...
| D-20  | F-20 Catch-up watcher                    | Не реализовано. Текущий F-04 watcher работает только в живом режиме (DispatchSource), пропуски при закрытой игре не догоняются | Единый 5-мин Timer для F-18/F-19; immediate scan при старте; per-source `last_check_ts` в Application Support; интервал переопределяемый в Settings (3–60 мин) | S |
...
С учётом закрытия D-06/D-07/D-08 в цикле 2026-05-22 и нового концепт-апдейта (F-17..F-20):

1. **D-20** (catch-up watcher, S) — фундамент для F-18 и F-19, единый Timer + per-source
   state. Малый объём, начать с него.
2. **D-17** (in-app journal, M) — самая приятная UX-часть, быстро виден результат.
   Не зависит ни от F-18/F-19, ни от D-02.
3. **D-18** (notes watcher, M) — заместит F-04 (TasksJsonlWatcher), требует D-20 (poll
   механизм). Шаблоны парсинга + опции dedup/удаление.
4. **D-19** (git watcher, M) — авто-учёт прироста кода, требует D-20.
5. **D-11** (журнал инспектора, остаток) — TASK-014 (население per-project, deferred,
   split-needed) + TASK-015 (фильтр по типу события, deferred, blocked by TASK-008).
6. **D-02** (SKTileMapNode, M, P0) — переход рендера земли. Pre-req для D-15 биомов.
7. **D-15** (биомы, L) — после D-02. Нужно разбиение в /pm.
8. **D-16** (50 юнитов, L) — после D-15 (нужен `terrain`-аффинитет). Нужно разбиение.

**Сходимость:** 11/18 в diff (9/18 закрыто полностью, новые F-17..F-20 добавлены как
