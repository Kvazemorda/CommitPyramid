...
_Обновлено: 2026-05-26 (sync — diff = 0, все 25 фич ✅; новые BUG-024/025 из TASK-056 Followups зафиксированы в Bugs.md)_
...
| ~~D-15~~ | ~~F-15 Биомы и генерация карты~~ | ✅ Закрыт: TASK-026/027/028/029 (шумовая основа + классификатор + рендер + зум) + TASK-030a/030b/030c (Settings UI + atomic reinit orchestrator + biome-aware district allocation). **Followup (отдельная задача):** в `isNewProject` ветке call site `unitIds=[]` → preferred=[]; реальный PM-сценарий «рыболовный квартал у реки после reinit» требует replay-aware integration. | — | M |
| ~~D-16~~ | ~~F-16 Расширенный каталог (50 юнитов)~~ | ✅ Закрыт: TASK-031..039 + TASK-040 (docs/asset-prompts.md — 67KB каталог промптов для AI-генерации спрайтов готов вместе с F-21 docs/). Reconciliation 2026-05-25 (unclaimed-progress: ⚠️→✅; коммит закрытия — в окне до 2f8a7db, sync-state отставал). | — | S |
| ~~D-21~~ | ~~F-21 Open-source готовность~~ | ✅ Закрыт 2026-05-23 (commit до окна 2f8a7db): MIT LICENSE, английская docs/ (README, concept, architecture, asset-prompts, log-format, sprite-generation-rules, CONTRIBUTING*), GitHub Kvazemorda/CommitPyramid public, без персональных данных. Reconciliation 2026-05-25 (unclaimed-progress: ❌→✅). | — | M |
...
| D-15  | F-15  | 2026-05-24 | `UI/SettingsView.swift` (секция «Карта мира»), `Data/AppSettings.swift` (v5 + worldSeed/pendingMapResetSeed), `Game/MapReinitCoordinator.swift` (atomic pause→bak→regenerate→replay→resume), `Game/TerrainAffinity.swift` (preferredBiomes helper), `Game/DistrictPlanner.swift` (allocateNextOrigin preferredBiomes filter), `Game/CityEngine.swift` (integration point); TASK-030a/030b/030c. Followup на replay-aware preferred integration — отдельная задача. |
...
4. ~~**D-02** (SKTileMapNode, M, P0)~~ — закрыт TASK-025.
5. ~~**D-15** (биомы, L)~~ — закрыт TASK-026..029 + TASK-030a/030b/030c (2026-05-24).
6. **D-16** (50 юнитов) — фактически закрыт TASK-031..039; остался TASK-040 (контент-арт, blocked-by F-21).

**Сходимость:** 0 расхождений (sync 2026-05-25: D-15 ✅ TASK-030*, D-16 ✅ TASK-040 docs/asset-prompts.md, D-21 ✅ docs/+LICENSE 2026-05-23, F-22 ✅ AppleScript scanAppleNotesFolder; диффы 16/21 закрыты ретро через reconciliation, F-22 не имел открытого D-NN — был в Current ❌, переведён в ✅).
