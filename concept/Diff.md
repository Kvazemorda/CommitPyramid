...
| D-25  | F-25 District templates + эпохи | Частично реализовано (5/7 sub-task'ов): TASK-047 ✅ (фундамент + 5 egyptian JSON), TASK-048a ✅ (ProjectState + AppSettings v4), TASK-048b ✅ (Picker + preferredSlotRole), TASK-048c ✅ (slot-placement integration + BUG-010 close in templated mode), TASK-049 ✅ (stage-up template migration + GameEvent.Kind.templateMigrated + step-wise loop + GameScene road-extension). **Остаток:** TASK-050 (era progression 0-3 + monumental после stage 5), TASK-051 (Settings UI «Стиль города» + AppDelegate wire). | Остаток: era progression + Settings UI. Replay детерминирован ✅. | L |
