...
| D-25  | F-25 District templates + эпохи | Не реализовано. Сейчас `UnitPlanner.nextPosition` ставит здания «depth=1 от road» через `extendDistrictPlan` — квартал выглядит как случайный сгусток вокруг road-петель, без читаемого силуэта города. Эволюция работает только через подмену kind (F-23), форма квартала не меняется при stage-up, после stage 5 город визуально не улучшается → отсутствует долгоиграющая награда для multi-year проектов. | Resources/DistrictTemplates/ содержит ≥15 JSON (3 family × 5 stage). Новый квартал собирается по выбранному шаблону (slot-based placement). Stage-up подменяет шаблон, юниты остаются в позициях. После stage 5 + 100 задач + 30 дней — era 1 с уникальным monumental-зданием. Settings → «Стиль города» Picker (auto/egyptian/roman/greek/mixed). Replay детерминирован. | L |
