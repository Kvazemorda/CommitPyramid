...
| F-16 | Расширенный каталог юнитов (50 шт.)           | ⚠️     | `Data/CityState.swift`                    | Каталог расширен до 51 case (50 из F-16 + warehouse legacy): BiomeKind, GridSize, UnitKindInfo, catalog словарь. Визуальные placeholder'ы для новых юнитов (TASK-032), terrain-аффинитет (TASK-033), эволюция в runtime (TASK-034), обновление UnitPlanner (TASK-035) — следующие подзадачи D-16. |
| F-17 | In-app journal (ручной ввод)                  | ❌     | —                                         | Не начато. Требует: глобальное окно журнала с dropdown projectId + Enter, контекстный клик по кварталу в explore, запись в `events.jsonl` source=journal |
...
**Сходимость:** 10/18 фич закрыто (56%) + 7 частично (F-17..F-19 как новые ❌).
