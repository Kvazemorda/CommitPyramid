...
| F-15 | Биомы и генерация карты                       | ⚠️     | `World/NoiseMap.swift`, `World/NoiseFieldGenerator.swift`, `World/BiomeKind.swift`, `World/BiomeClassifier.swift` | Шумовые поля 256×256 (TASK-026 ✅). BiomeKind enum (7 биомов) + BiomeClassifier: квантильные пороги, flood-fill sea, downhill rivers (TASK-027 ✅). Рендер биомов — TASK-028 ❌. Реинициализация карты — TASK-030 ❌. Зум — TASK-029 ❌. |
| F-16 | Расширенный каталог юнитов (50 шт.)           | ⚠️     | `Data/CityState.swift`, `Data/SnapshotStore.swift`, `Tests/.../LegacyStateMigrationTests.swift`, `Game/CityEngine.swift`, `Game/GameScene.swift`, `Data/GameEvent.swift` | Каталог расширен до 51 case (50 из F-16 + warehouse legacy): BiomeKind, GridSize, UnitKindInfo, catalog словарь. Миграция legacy state 12→50 готова (TASK-037). Эволюционные цепочки (TASK-034) ✅: GameEvent.Kind.unitEvolved, applyEvolutionsIfReady repeat-loop, UnitState.kind var, onUnitEvolved callback, GameScene.handleUnitEvolved/swapEvolvedSprite. terrain-аффинитет (TASK-033), обновление UnitPlanner (TASK-035), финальные спрайты (TASK-040) — следующие подзадачи D-16. |
...
**Сходимость:** 12/18 фич закрыто (67%) + 5 частично (F-18 и др. ❌).
