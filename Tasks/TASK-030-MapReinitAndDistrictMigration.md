...
_Дата: 2026-05-23_
_Модель: opus_
_Статус: [x] escalate-too-large → resolved-via-spec (2026-05-23 PM решение)_

### 📌 Продуктовое решение по replay-контракту (PM, 2026-05-23)

Из 3 предложенных вариантов (A: event-carries-map / B: deterministic-migration / C: hybrid)
выбран **B+C-гибрид с явным confirm от юзера**:

- При нажатии «Новая карта» в Settings:
  1. Показать confirm-dialog: «Старая карта будет стёрта. Город пересоберётся с нуля
     из существующих логов задач (`events.jsonl`). Продолжить?»
  2. По confirm: новый seed → сохранить → `snapshot.json` удалить → `eventsSinceSnapshot = 0`.
  3. Перезапустить `replayFromLog()` от пустого state с новой картой.
  4. Кварталы пересоберутся на новых координатах: каждый старый `ProjectState`
     получит новый `districtOrigin` через `DistrictPlanner.allocateNextOrigin` в новой топологии.
- НЕТ записи `map_regenerated` события в `events.jsonl` (лог чистый).
- НЕТ миграции snapshot v1→v2 (формат не меняется, snapshot просто перезаписывается).
- Реплей из старого лога после смены seed даёт **другую** визуально карту с теми же кварталами —
  это ожидаемое поведение, юзер был предупреждён.

**Этот спек разблокирует split на atomic-подзадачи через /pm в следующем /sync.**
Lead-разбор ниже отражает _старое_ состояние «нет блокеров не пройдено» — после
закрытия всей цепочки 026/027/028/029 PM делает split на 030a–030d.
